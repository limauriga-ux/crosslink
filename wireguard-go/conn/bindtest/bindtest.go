...
func (c *ChannelBind) Open(port uint16) (fns []conn.ReceiveFunc, actualPort uint16, err error) {
	...
	fns = append(fns, c.makeReceiveFunc(signal, *c.rx4))
	fns = append(fns, c.makeReceiveFunc(signal, *c.rx6))
	if rand.Uint32()&1 == 0 {
...
func (c *ChannelBind) makeReceiveFunc(signal <-chan struct{}, ch chan []byte) conn.ReceiveFunc {
