...
func (tun *netTun) Read(buf [][]byte, sizes []int, offset int) (int, error) {
	...
	// Do not deliver a packet that was queued before teardown became visible.
	select {
	case <-tun.done:
		return 0, os.ErrClosed
	default:
...
func (tun *netTun) Close() error {
	tun.closeOnce.Do(func() {
		// Publish shutdown before touching the stack: a callback may be
		// holding a gVisor endpoint lock while waiting to hand off a view.
		close(tun.done)
		...
		// incomingPacket intentionally remains open. Readers stop through
		// done/EOF, and late callbacks can safely take the cancellation arm.
		for {
			select {
			case view := <-tun.incomingPacket:
				if view != nil {
